            ...
            if (!states.count(temp) && !temp.empty()) {
                ...
                for (int i = 0; i < DFAStateNum; i++) {
                    if (temp == DFAStates[i].closure_) {
                        DFAStates[num].edges[DFAStates[num].edgeNum_].input_ = e;		
                        DFAStates[num].edges[DFAStates[num].edgeNum_].trans_ = i;			
                        DFAStates[num].edgeNum_++;										

                        dfa.trans_[num][e - 'a'] = i;	
                        break;
                    }
                }
            }
...
void DFA::printDFA(DFA d) {
    ...
    int i, j;
    std::cout << "DFA всего имеет " << DFAStateNum << " состояний, " << "начальное состояние - " << d.startState_ << std::endl << std::endl;

    std::cout << "Конечный алфавит: { ";
    ...
    std::cout << "Множество конечных состояний: { ";
    ...
    std::cout << "Функция переходов:" << std::endl;
    ...
    std::cout << std::endl << "Матрица переходов:" << std::endl << "     ";
